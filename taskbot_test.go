...
func wait(n time.Duration) {
	// увеличил таймаут, т.к. то встроенный терминал VSC подтупливает при логировании диалога
	const k = 20 * time.Millisecond
	time.Sleep(n * k)
}
...
func TestTasks(t *testing.T) {
	...
	defer cancel()
	go func() {
		// если не указать 127.0.0.1, то MD дергает файрвол и сервер не успевает подняться
		err := startTaskBot(ctx, "127.0.0.1:8081")
